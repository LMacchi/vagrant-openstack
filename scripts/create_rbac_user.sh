...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 /path/to/user/data/file.json"
  exit 1
fi
...
curl -s -X POST --data @$DATA -H "Content-Type: application/json" --cert $CERT --key $KEY --cacert $CACERT https://${MASTER}:4433/rbac-api/v1/users
